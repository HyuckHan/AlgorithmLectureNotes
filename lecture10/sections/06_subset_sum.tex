...
\centering\begin{tikzpicture}[level distance=11mm,sibling distance=34mm]
\node[ssnode]{\(i0,s0,r18\)}
 child{node[sscurrent]{\(\{3\},s3\)} child{node[ssnode]{\(\{3,4\},s7\)} child{node[ssdead]{\(s12\)}} child{node[ssvalid]{\(\{3,6\}\)}}} child{node[ssdead]{\(\{3\},r5\)}}}
 child{node[ssnode]{\(\{\},s0\)} child{node[sscurrent]{\(\{4\},s4\)} child{node[ssvalid]{\(\{4,5\}\)}} child{node[ssdead]{\(r6<9\)}}}};
\end{tikzpicture}
\end{frame}
