...
\draw[ssactiveedge](c)--node[above,font=\small]{Recurse}(r);
\draw[ssactiveedge](r)to[bend left=35]node[below,font=\small]{Undo \(d\): exactly \(S\)}(s);
\node[ssnote]at(0,-.5){\only<1|handout:0>{decision을 state에 반영}\only<2|handout:0>{promising이면 재귀}\only<3|handout:1>{return 후 Undo: sibling은 동일한 parent state에서 시작}};
\end{tikzpicture}
\end{frame}
