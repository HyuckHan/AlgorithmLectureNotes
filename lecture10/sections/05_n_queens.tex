...
\only<1|handout:0>{\node[ssnote]at(6,1.5){해 \([1,3,0,2]\)를 한 row씩 확인한다};}
\visible<2->{\node[board,fill=AlgoOrange!18]at(1,3){Q};}
\only<2|handout:0>{\node[ssnote]at(6,1.5){row 0: col 1};}
\visible<3->{\node[board,fill=AlgoBlue!15]at(3,2){Q};}
\only<3|handout:0>{\node[ssnote]at(6,1.5){row 1: col 3 --- row 0과 열·대각선 무충돌};}
\visible<4->{\node[board,fill=green!12,draw=green!45!black,ultra thick]at(0,1){Q};}
\only<4|handout:0>{\node[ssnote]at(6,1.5){row 2: col 0 --- 앞의 두 퀸과 무충돌};}
...
\centering\resizebox{!}{5.3cm}{\begin{tikzpicture}[level distance=9mm,level 1/.style={sibling distance=30mm},level 2/.style={sibling distance=18mm},level 3/.style={sibling distance=14mm},level 4/.style={sibling distance=12mm}]
\node[ssnode]{[]}
 child{node[ssnode]{[0]}
   child{node[ssdead](d1){[0,2]}}
   child{node[ssnode]{[0,3]}
     child{node[ssdead](d2){[0,3,1]}}
   }
 }
 child{node[sscurrent]{[1]}
   child{node[sscurrent]{[1,3]}
     child{node[sscurrent]{[1,3,0]}
       child{node[ssvalid]{[1,3,0,2]}}
     }
   }
 }
 child{node[ssnode]{[2]}
   child{node[font=\scriptsize,text=AlgoGray]{\(\cdots\)} edge from parent[dashed]
     child{node[ssvalid,solid]{[2,0,3,1]} edge from parent[solid]}
   }
 }
 child{node[ssnode]{[3]}
   child{node[font=\scriptsize,text=AlgoGray]{\(\cdots\)} edge from parent[dashed]}
 };
\node[font=\scriptsize,text=red!70!black,below=.5mm of d1]{row2 후보 소진};
\node[font=\scriptsize,text=red!70!black,below=.5mm of d2]{row3 후보 소진};
\node[font=\scriptsize\bfseries]at(0,-4.4){두 solution: \([1,3,0,2]\), \([2,0,3,1]\)};
\node[font=\scriptsize\bfseries]at(0,-4.8){표시되지 않은 형제는 promising 검사에서 즉시 기각된 후보다};
\end{tikzpicture}}
\end{frame}
