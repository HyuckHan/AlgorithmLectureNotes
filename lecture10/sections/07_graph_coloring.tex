...
% Drawn instead of \times: the trace frame repeats this glyph up to 3
% times per overlay state, which makes dvisvgm's <defs> glyph-collection
% order for that glyph nondeterministic across otherwise-identical
% compiles. A path has no glyph entry, so the byte output is stable.
% (Defined inside the tikzpicture, not before the frame, because
% extract_tikz.py's TIKZ_RE only captures \begin{tikzpicture}..\end{tikzpicture}
% verbatim -- a \newcommand outside that span is invisible to the pipeline.)
\newcommand{\rejectmark}{\tikz[baseline=(current bounding box.center)]{\draw[red!60!black,line width=.5pt](0,0)--(1.3mm,1.3mm);\draw[red!60!black,line width=.5pt](0,1.3mm)--(1.3mm,0);}}
