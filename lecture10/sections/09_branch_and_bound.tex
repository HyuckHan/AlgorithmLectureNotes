...
\node[ssnote]at(0,-3){\only<1|handout:0>{root 확장}\only<2|handout:0>{feasible solution 발견}\only<3|handout:0>{incumbent 갱신}\only<4|handout:1>{UB 12인 subtree는 incumbent 14를 넘을 수 없어 prune}};
\end{tikzpicture}
\end{frame}
