...
\end{frame}
\begin{frame}{Graph-Search A*: 메인 루프}
\algofont
\begin{algorithmic}[1]
...
\end{algorithmic}
\vfill
\muted{OPEN에서 \(f=g+h\)가 최소인 state를 꺼내 CLOSED로 확정한다.}
\end{frame}

\begin{frame}{Graph-Search A*: 이웃 완화(Relax)}
\algofont
\begin{algorithmic}[1]
\Procedure{Relax}{$u$}
...
\vfill
\begin{block}{본문 정책}
integer edge cost와 integer \(h\), nonnegative cost, consistent \(h\).
일반 floating-point 구현은 exact \(f\) equality 대신 저장한 \(g\) snapshot(또는 version)을 검사한다.
\end{block}
\end{frame}
