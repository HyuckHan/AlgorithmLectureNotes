...
\node[ssnode,label=above:{root: 결정 0개}]{\(\varnothing\)}
 ...
 child{node[ssnode]{B} child{node[ssnode]{B,C} edge from parent node[left,font=\tiny]{choose C}} child{node[ssbound]{B,D} edge from parent node[right,font=\tiny]{choose D}} edge from parent node[right,font=\tiny]{choose B}};
