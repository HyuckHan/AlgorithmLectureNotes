...
\end{tikzpicture}
\vspace{2mm}\only<1|handout:0>{5부터 선택}\only<2|handout:0>{다음은 8}\only<3|handout:0>{5, 8 뒤에는 10}\only<4|handout:0>{다음은 13}\only<5|handout:0>{13 다음 14}\only<6|handout:0>{21로 R이 소진된다}\only<7|handout:0>{R이 비었으므로 L의 29, 37을 그대로 옮긴다}\only<8|handout:1>{한쪽이 비면 나머지 suffix를 붙인다.}
\end{frame}
\begin{frame}{Merge Sort의 시간}
\begin{columns}\column{.5\textwidth}\begin{tikzpicture}[level distance=9mm,level 1/.style={sibling distance=28mm},level 2/.style={sibling distance=14mm},every node/.style={font=\small}]
...
\end{frame}
\begin{frame}{다시 만나는 Master Theorem}
Lecture 2의 \keyterm{Master Theorem}이면 같은 답을 재귀 트리 없이 즉시 분류한다.
\[
T(n)=2\,T(n/2)+\Theta(n),\qquad a=2,\ b=2.
\]
...
\end{columns}
\vspace{2mm}
두 비용이 같은 차수이므로 \keyterm{Case 2}에 해당하고, 로그 한 겹이 붙는다.
\[
T(n)=\Theta\!\left(n^{\log_b a}\log n\right)=\Theta(n\log n).
\]
\begin{takeaway}재귀 트리로 유도한 \(\Theta(n\log n)\)과 Master Theorem Case 2는 같은 결론이다. 도구를 바꿔도 답은 변하지 않는다.\end{takeaway}
\end{frame}
