...
\only<1>{\def\vals{{29,10,14,37,13}}\def\a{1}\def\b{2}}
\only<2>{\def\vals{{10,29,14,37,13}}\def\a{2}\def\b{3}}
\only<3>{\def\vals{{10,14,29,37,13}}\def\a{4}\def\b{5}}
\only<4>{\def\vals{{10,14,29,13,37}}\def\a{0}\def\b{0}}
\foreach \i in {1,...,5}{\pgfmathparse{\vals[\i-1]}\edef\v{\pgfmathresult}\ifnum\i=\a\node[sort current]at(\i,0){\v};\else\ifnum\i=\b\node[sort current]at(\i,0){\v};\else\ifnum\i=5\ifnum\a=0\node[sort done]at(\i,0){\v};\else\node[sort cell]at(\i,0){\v};\fi\else\node[sort cell]at(\i,0){\v};\fi\fi\fi}
\end{tikzpicture}
\vspace{4mm}\only<1|handout:0>{29와 10: 역전이므로 swap}\only<2|handout:0>{29와 14: swap}\only<3|handout:0>{37과 13: swap}\only<4|handout:1>{첫 pass 종료: 최댓값 37 확정}
\end{frame}
