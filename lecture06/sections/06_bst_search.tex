...
\node[st current]{15}child{node[st inactive]{6}}child{node[st done]{18}child{node[st current]{17}}child{node[st node]{20}}};
\end{tikzpicture}
\only<1|handout:0>{\begin{block}{Case 1}$succ(15)=min(18\ subtree)=17$\end{block}}
\only<2|handout:1>{\begin{block}{Case 2}$succ(4)=6$: 4에서 올라가 처음 left-turn 경계를 찾음\end{block}}
\column{.48\textwidth}
\only<1|handout:0>{right subtree가 있으므로 그 안의 minimum}
\only<2|handout:1>{\begin{tikzpicture}\node[st done](a)at(0,0){6};\node[st node](b)at(-1,-1){3};\node[st current](c)at(.3,-2){4};\draw[st active edge](c)--(b)--(a);\end{tikzpicture}}
\end{columns}
\end{frame}
...
\]
\begin{itemize}\item parent pointer가 있으면 upward walk가 직접 가능\item 없으면 root부터 search path를 기억하거나 다시 탐색\item predecessor는 left/right와 min/max를 바꾼 symmetric operation\end{itemize}
\end{frame}
