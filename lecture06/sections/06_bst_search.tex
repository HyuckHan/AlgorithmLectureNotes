...
\node[st node](n15){15}
 child{node[st node](n6){6}
   child{node[st node](n3){3}
     child{node[st node](n2){2}}
     child{node[st node](n4){4}}
   }
   child{node[st node](n7){7}
     child[missing]{}
     child{node[st node](n13){13}
       child{node[st node](n9){9}}
       child{node[st node](n14){14}}
     }
   }
 }
...
\only<1|handout:0>{
\node[st current]at(n15){15};
\node[st inactive]at(n6){6};\node[st inactive]at(n3){3};\node[st inactive]at(n2){2};\node[st inactive]at(n4){4};
\node[st inactive]at(n7){7};\node[st inactive]at(n13){13};\node[st inactive]at(n9){9};\node[st inactive]at(n14){14};
...
\end{tikzpicture}
\only<1|handout:0>{%
\small\begin{block}{Case 1}$succ(15)=\min(18\ subtree)=17$\end{block}
\centering\footnotesize right subtree가 있으므로 그 안의 minimum:
$x.right\ne\NIL,\ \operatorname{successor}(x)=\operatorname{minimum}(x.right)$.
}
\only<2|handout:1>{%
\small\begin{block}{Case 2: right subtree가 없을 때}
\footnotesize right subtree가 없으면 부모를 올라가며 현재 node가 처음으로 ancestor의 left subtree에
놓이는 그 ancestor다. $succ(4)=6$.
\end{block}
}
\end{frame}
...
\]
\begin{itemize}
\item parent pointer가 있으면 upward walk가 직접 가능하고, 없으면 root부터 search path를 기억하거나 다시 탐색한다.
\item predecessor는 left/right와 min/max를 바꾼 symmetric operation이다.
\end{itemize}
\end{frame}
