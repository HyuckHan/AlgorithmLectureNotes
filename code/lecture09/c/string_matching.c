...
static bool valid_args(const unsigned char *t, size_t n,
                       const unsigned char *p, size_t m, const SmMatches *out) {
    return out && (n == 0 || t) && (m == 0 || p);
}
...
bool sm_naive_all(const unsigned char *t, size_t n,
    ...
    if (!valid_args(t,n,p,m,out) || !begin_output(out,m)) return false;
...
bool sm_rabin_karp_all(const unsigned char *t, size_t n,
    ...
    if (!valid_args(t,n,p,m,out) || base == 0 || q <= 1 || !begin_output(out,m)) return false;
    if (m == 0 || m > n) return true;
...
bool sm_kmp_all(const unsigned char *t, size_t n,
    ...
    if (!valid_args(t,n,p,m,out) || !begin_output(out,m)) return false;
    if (m == 0 || m > n) return true;
    if (m > SIZE_MAX/sizeof(size_t)) return false;
...
size_t sm_naive_first(const unsigned char *t, size_t n,
                      const unsigned char *p, size_t m) {
    if ((n && !t) || (m && !p)) return SM_NOT_FOUND;
    if (m==0) return 0;
    if (m>n) return SM_NOT_FOUND;
    for (size_t s=0;s<=n-m;s++) if (equal_at(t,p,s,m)) return s;
    return SM_NOT_FOUND;
}
size_t sm_kmp_first(const unsigned char *t, size_t n,
                    const unsigned char *p, size_t m) {
    SmMatches out={0};
    if (!sm_kmp_all(t,n,p,m,&out)) return SM_NOT_FOUND;
    size_t first=out.count?out.data[0]:SM_NOT_FOUND;
    sm_matches_destroy(&out); return first;
}
